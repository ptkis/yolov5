...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(
        ...
        if exists and LOCAL_RANK in {-1, 0}:
            ...
            tqdm(None, mininterval=300.0, desc=prefix + d, total=n, initial=n, bar_format=TQDM_BAR_FORMAT)  # display cache results
            if cache["msgs"]:
...
class HUBDatasetStats:
    ...
    def process_images(self):
        ...
        for split in "train", "val", "test":
            ...
            dataset = LoadImagesAndLabels(self.data[split])  # load dataset
            desc = f"{split} images"
            for _ in tqdm(ThreadPool(NUM_THREADS).imap(self._hub_ops, dataset.im_files), total=dataset.n, mininterval=300.0, desc=desc):
                pass
